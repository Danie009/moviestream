    ...
    >
      <CardContent className="p-0 h-full relative">
        ...
        <div className="relative w-full h-full">
          ...
          <div className="hidden sm:block absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/60 to-transparent opacity-0 group-hover:opacity-200 transition-opacity duration-300 flex flex-col justify-end p-4">
            {/* Movie Title */}
            <div className="mb-4">
              <h3 className="text-white font-bold text-lg leading-tight line-clamp-2 mb-1">{movie.title}</h3>
              {/* Apply truncation to the description */}
              <p className="text-white/80 text-sm">{truncateWords(movie.description, 11)}</p>
            </div>
