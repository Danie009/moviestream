...
export default function MoviesManagementPage() {
  ...
  useEffect(() => {
    const loadedMovies = movieService.getAllMovies() // Changed from getMovies() to getAllMovies()
    setMovies(loadedMovies)

    // Listen for movie updates
    const handleMoviesUpdate = (event: CustomEvent) => {
      setMovies(event.detail)
    }
  ...
  const toggleStreaming = (movieId: string) => {
    movieService.toggleStreaming(movieId)
  }

  const scheduleRemoval = (movieId: string) => {
    movieService.scheduleRemoval(movieId)
  }

  const cancelScheduledRemoval = (movieId: string) => {
    movieService.cancelScheduledRemoval(movieId)
  }
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {Math.round((streamingCount / totalCount) * 100)}% of library
            </p>
          </CardContent>
        </Card>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            {movie.scheduledRemoval ? (
                              <DropdownMenuItem onClick={() => cancelScheduledRemoval(movie.id)}>
                                <X className="h-3 w-3 mr-2" />
                                Cancel Scheduled Removal
                              </DropdownMenuItem>
                            ) : (
                              <>
                                <DropdownMenuItem onClick={() => toggleStreaming(movie.id)}>
                                  {movie.isStreaming ? (
                                    <>
                                ...
                                {movie.isStreaming && (
                                  <DropdownMenuItem onClick={() => scheduleRemoval(movie.id)}>
